...
half4 frag(v2f i) : SV_Target
{
    return skybox_frag(i, _BackTex, _BackTex_HDR, _BackTex2, _BackTex2_HDR);
}
        ENDCG
    }
    Pass{
...
half4 frag(v2f i) : SV_Target
{
    return skybox_frag(i, _LeftTex, _LeftTex_HDR, _LeftTex2, _LeftTex2_HDR);
}
        ENDCG
    }
    Pass{
...
half4 frag(v2f i) : SV_Target
{
    return skybox_frag(i, _UpTex, _UpTex_HDR, _UpTex2, _UpTex2_HDR);
}
        ENDCG
    }
    Pass{
...
half4 frag(v2f i) : SV_Target
{
    return skybox_frag(i, _DownTex, _DownTex_HDR, _DownTex2, _DownTex2_HDR);
}
        ENDCG
    }
}
